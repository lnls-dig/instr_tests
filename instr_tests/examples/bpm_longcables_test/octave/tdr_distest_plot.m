...
ref_dist = tdr_time2dist(ref_dly, diel)/1e-3;
...
xlabel(sprintf('Distance relative to %0.2f mm [mm]', ref_dist), 'FontSize', 16);
for i=1:size(data,2)
    leg{i} = sprintf('Cable #%d, \\Deltat_{%d\\rightarrow%d}: %0.2f mm\n', i, ref_i, i, dist_cable(i));
end
